...
# from swin_Den4KVStage3q_sota_14 import *
from .PintView_GCN import *
...
class PointNet(ExtendedModule):
    def __init__(
        ...
        #改寫部分
        # self.GSNet = SwinTransformer(hidden_dim=96, layers=(2, 2, 6, 2), heads=(3, 6, 12, 24),window_size=1)  #创建对象
        self.C1 = nn.Conv1d(6, 1, kernel_size=1, stride=1, bias=False)
        self.B1 = nn.BatchNorm1d(1)
        self.R = nn.ReLU(inplace=True)
        self.L1 = nn.LayerNorm(2048)
        self.C2 = nn.Conv1d(1, 512, kernel_size=1, stride=1, bias=False)
        self.B2 = nn.BatchNorm1d(512)


        # self.GCN = PointViewGCN(name="log", nclasses=1, num_views=20)
    ...
    def forward(self, inputs, object_feature=True, concat_state=None, **kwargs):
        ...
        if 2 in self.feature_transform:
            ...
            print(feature.shape)
            bs = feature.shape[0]
            # feature = torch.reshape(feature, (bs, 6, 32, 32))
            # feature = self.C1(feature)
            # feature = self.GSNet(feature)
            # feature= torch.reshape(feature, (bs, 1, 64, 64))
            # feature= self.C2(feature)
            # feature = torch.reshape(feature, (bs, 512, 1024))

            feature = self.C1(feature)
            feature = self.B1(feature)
            feature = self.R(feature)
            GCN = PointViewGCN(name="log", nclasses=1024, num_views=bs)
            GCN = GCN.to('cuda')
            feature = GCN(feature)
            feature = self.L1(feature)
            out_channel = 512 * bs
            Lin = nn.Linear(2048, out_channel, bias=True)
            Lin = Lin.to('cuda')
            feature = Lin(feature)
            feature = torch.reshape(feature, (bs, 1, 512))
            feature = self.C2(feature)
